# Origianl Sequence github repo https://github.com/heksadecimal/sequence.git
# Modified game.py for the agent-driven environment
# Added begin_game() and agent_move() functions for the RandomAgent to play
# Fixed float-to-int issues from the original code for QRect and QPoint
# Fixed one-eye jack bug from the original code because the move wouldn't result in 
...
_QRect = QRect
def QRect(x=None, y=None, w=None, h=None, **kw):
    if x is not None:
        return _QRect(int(x), int(y), int(w), int(h))
    return _QRect()

_QPoint = QPoint
def QPoint(x=None, y=None):
    if x is not None:
        return _QPoint(int(x), int(y))
    return _QPoint()
...
class Game_Renderer:
    ...
    def render(self) -> QWidget:
        ...
        self.menuPushButton.setGeometry(
            QRect(0.04 * self.mainPage.width(), 0.12 * self.mainPage.height(), 120, 90)
        )
    ...
    def declare(self, status, who=""):
        ...
        flash.setGeometry(
            QRect(0.35 * self.mainPage.width(), 0.4 * self.mainPage.height(), 700, 300)
        )
    ...
    def begin_game(self):
        ...
        from agent import ExpectiminimaxAgent
        self.agent = ExpectiminimaxAgent(depth=2)
        self.timer = QTimer()
    ...
    def agent_move(self):
        """
        Once the begin_game timer times out, checks the agent's legal moves
        based on cards in its hand,
        then simulates a click on the respective card icon to execute the move.
        """
        actions = []
        hand = self.challenger.playerCards
        self.agent.set_state(self.game, self.challenger, self.bot)


        #Valid moves from backend/logic.py makeRandomMove function
        for card in set(hand):
            if card in ("JH", "JS"):
                #One-eyed jack, removes any opponent token on board
                for i in range(10):
                    for j in range(10):
                        if self.bot.playerBox[i][j]:
                            actions.append((card, i, j))
            elif card in ("JD", "JC"):
                #Two-eyed jack, wild card
                for i in range(10):
                    for j in range(10):
                        if (self.game.board[i][j] != "XX"
                                and self.challenger.playerBox[i][j] == 0
                                and self.bot.playerBox[i][j] == 0):
                            actions.append((card, i, j))
            else:
                #Normal card
                for i in range(10):
                    for j in range(10):
                        if (self.game.board[i][j] == card
                                and self.challenger.playerBox[i][j] == 0
                                and self.bot.playerBox[i][j] == 0):
                            actions.append((card, i, j))
        ...
        card_icon = self.revposition[(x, y)]
        self.click(card_icon)

        #If the game ends, stop the timer
        if self.game.winner or not self.game.deck:
            self.timer.stop()
